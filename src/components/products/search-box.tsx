...
export function SearchBox() {
  ...
  const listingPath = localePath(locale, "/products");
  const isListing = pathname === listingPath || pathname === "/products";
  ...
  }, [value, isListing, listingPath, searchParams, router]);
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(() => {
      router.push(
        trimmed ? `${listingPath}?q=${encodeURIComponent(trimmed)}` : listingPath,
      );
    });
  };
  ...
  return (
    <form role="search" onSubmit={handleSubmit} className="relative w-full">
      ...
      <SearchIcon className="pointer-events-none absolute left-4 top-1/2 size-4.5 -translate-y-1/2 text-ink-muted" />
      <input
        id={inputId}
        type="search"
        name="q"
      ...
      {isPending && (
        <span
          role="status"
          aria-label={dict.header.searchPending}
